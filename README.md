...
**Current status / 当前状态**: management layer complete + P4 polish — **v0.3.0-p4c**. Web console (Vue3 + Element Plus + ECharts): dashboard with real-time alert stream, device ledger with metric trends, alert center (one-click → work order, delete/clear), work-order kanban with FSM-enforced transitions, maintenance plan/record management with due highlighting, MCP registry with runtime quick-config, external API onboarding page, secretary-agent chat with tool-trace timeline, knowledge base (search / file import), data cleanup, in-app usage guide. Backend: resource API + delete/cleanup, knowledge base + agent tools (`search_knowledge`, `get_fault_report`), WebSocket (`/ws/dashboard`, `/ws/chat`), maintenance reminders, external API onboarding with inbound API-key auth, hidden-window quick start. 327 tests green, ruff clean, web build green, golden evaluation 30/30. One-command demos: `bash scripts/demo_p1.sh` · `bash scripts/demo_p2.sh` · `bash scripts/experiment_p3.sh`. / 管理层完成 + P4 打磨——**v0.3.0-p4c**。Web 控制台：总览大盘（实时告警流）、设备台账（指标趋势）、告警中心（一键转工单/删除/清空）、工单看板（状态机校验）、维保管理（到期高亮）、MCP 服务（页内快捷注册/移除）、API 接入管理、智能问答（工具轨迹时间线）、知识库（搜索/文件导入）、数据清理、使用说明。后端：资源 API + 删除/批量清理、知识库 + Agent 工具（search_knowledge / get_fault_report）、WebSocket、周期提醒、外部 API 接入（入站 Key 鉴权）、隐藏窗口一键启动。327 个测试全绿，ruff/构建全绿，golden 评测 30/30。一键演示 `bash scripts/demo_p1.sh`（设备层）/ `bash scripts/demo_p2.sh`（智能层）/ `bash scripts/experiment_p3.sh`（全链路模拟实验）。
...
- **API 接入**：控制台「API 接入」页管理外部端点（新增/启停/删除/LLM 降级链），并可在页内保存本前端 X-API-Key 以解锁入站鉴权。
- **知识库**：控制台「知识库」页搜索/导入/删除故障知识文档；内置 9 类故障剧本的原因与处理方法（启动时自动加载）。智能问答「球管过热怎么处理」会检索知识库；「给我 ct 的故障报告」聚合设备告警/异常日志/工单/维保记录。
- **后台静默运行**：start.bat 启动的 PostgreSQL 与后端进程完全隐藏（无控制台窗口），日志见 `deploy/api.log`；停止仍用「medops-停止」（按端口杀进程）。
