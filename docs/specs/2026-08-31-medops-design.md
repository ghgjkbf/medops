...
- 日期：2026-08-31
- 状态：v1（已经三轮评审确认）
- 用途：毕业设计（答辩 2028.5）+ 开源项目（GitHub 发布）
...
**论文版**：基于 MCP 协议与多 Agent 协作的医疗器械智能运维管理系统的设计与实现。
...
| # | 决策 | 理由 |
|---|------|------|
| D1 | 全模拟设备路线（自研模拟器，不接真实设备） | 毕设最常见、风险最低、答辩可控；检测逻辑真实可用 |
...
| D4 | MCP 部署形态选「拟真分布式」：每台模拟设备 = 独立进程 = 独立 MCP Server（streamable-http） | 还原真实医院拓扑（CT 在放射科、呼吸机在 ICU）；可演示拔设备离线告警；适合 docker compose 一键拉起；论文有三方案对比可写 |
| D5 | 模拟器与 MCP Server 分离（medops-sim / medops-engine 独立包） | 「接入新设备只需写一个 MCP Server + 一个模拟器」是开源生长点；故障注入引擎独立为第二个论文技术点 |
| D6 | 主库 PostgreSQL；`device_metric` 按 TimescaleDB hypertable 规范设计（兼容不实做） | 论文可提时序库升级路径且表述诚实成立 |
| D7 | DICOM 只做文件级（共享目录写入 .dcm + 元数据/完整性检测），不做 DICOM 网络协议 | 工作量边界，写进论文「研究范围」 |
...
- C 纯 function calling：最省事但选题不成立 → 仅作为论文对照方案
...
2. **问答链路**：用户提问 → 秘书 Agent 判断意图 → 调 MCP 工具查状态/日志/台账 → 结合知识库 → 回答；前端展示完整工具调用轨迹（答辩演示核心画面）。
3. **日志链路**：模拟器写日志文件 → tail 采集 → 规则引擎（阈值/关键字，毫秒级）→ 事件入库 → LLM 低频批量归因 → 预警分级。规则引擎负责「快」，LLM 负责「懂」，成本由此可控。
4. **配置下发链路**：管理 Agent 可通过 MCP 修改模拟器故障参数（故障剧本触发），一键造故障——答辩杀手锏，同时补全 MCP 写路径（带副作用的控制指令）故事。
...
3. **LLM 层**：降级链 DeepSeek → 备用 API → Ollama 本地 → 纯规则引擎结论（预警永不因 LLM 故障丢失，仅「归因」字段降级为规则摘要）
...
- **Agent 评测集**：30–50 个黄金场景（故障注入 → 期望工具调用序列 + 期望诊断结论）；脚本断言工具选择正确性，LLM-as-judge 评归因质量 → 产出「诊断准确率 / 工具命中率」等指标（论文实验章素材）
...
- 答辩前演练「断网剧本」：Ollama 本地模型 + 全部进程本机运行

## 12. 里程碑（2026.9 → 2028.5，阶段末必有可运行版本）

| 阶段 | 时间 | 交付物 |
|---|---|---|
| P0 设计与脚手架 | 2026.9–10 | 设计文档定稿、repo + CI、模拟器引擎原型（≈开题材料） |
| P1 设备层 | 2026.11–2027.1 | 4 类模拟器 + 故障剧本库、6 个 MCP Server（含 maintenance-db） |
| P2 智能层 | 2027.2–4 | 双 Agent、日志管道、预警引擎（≈中期检查素材） |
| P3 管理层 | 2027.5–7 | Web 后台、台账/工单/提醒闭环 → **v0.1.0 发布** |
| P4 打磨 | 2027.8–10 | 双语文档/Ollama 兜底/演示剧本/社区模板，v0.2 |
| P5 实验与论文 | 2027.11–2028.1 | 功能+性能测试、LLM 方案对比实验、论文初稿 |
| P6 收尾 | 2028.2–5 | 定稿、盲审修改、彩排、答辩 |
...
| 答辩现场断网/LLM 不稳定 | Ollama 本地兜底 + 断网演练剧本；规则引擎保证预警链路不依赖 LLM |
| 演示故障不可复现 | 故障剧本引擎（medops-engine）+ 配置下发一键造故障 |
| 「模拟数据说服力」质疑 | 检测逻辑真实（进程/文件/驱动级检测）；论文明确研究范围（D7）；三方案对比论证架构 |
| LLM 成本失控 | 规则引擎前置过滤 + LLM 低频批量归因；评测集脚本化，不烧 token 反复人工测 |

## 14. 范围边界（论文「研究范围」一节直接可用）
