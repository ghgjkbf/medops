...
const sections = [
  ...
  },
  {
    ...
    body: [
      ...
      '维保管理：周期维保计划与维保/维修记录。',
      'MCP 服务：设备接入服务（MCP Server）的注册状态与工具列表，支持页内快捷注册/移除。',
      '智能问答：与运维秘书对话，自动调用设备工具并展示调用轨迹；操作类指令（转工单/清理/触发巡检等）由管家 Agent 执行，高危操作需回复确认。',
      '知识库：故障原因与处理方法文档，支持搜索、文件导入（txt/md/csv/json）与删除；智能问答与巡检归因自动检索。支持绑定外部知识源（网页/RSS/向量库文件）定期或手动同步。',
  ...
  },
  {
    title: '知识库',
    body: [
      '内置 9 类故障剧本知识（CT 球管过热、PACS 断连、氧电池漂移、管路泄漏等）的原因与处理方法，平台启动时自动加载。',
      '「知识库」页可搜索（如输入"球管过热"）、导入 txt/md/csv/json 文件（≤512KB）、删除文档。',
      '智能问答输入"球管过热怎么处理"会自动检索知识库并给出处理步骤；"给我 ct 的故障报告"会聚合该设备的告警、异常日志、工单与维保记录。',
      '巡检产生告警时，归因信息会自动附加知识库建议，告警中心可直接看到处理指引。',
    ],
  },
  {
    title: '知识库',
    body: [
      '内置 9 类故障剧本知识（CT 球管过热、PACS 断连、氧电池漂移、管路泄漏等）的原因与处理方法，平台启动时自动加载。',
      '检索后端三层可切换：keyword（jieba+BM25，默认）/ vector（fastembed 中文向量 + sqlite-vec）/ legacy（2-gram 兜底）。',
      '「知识库」页可搜索（如输入"球管过热"）、导入 txt/md/csv/json 文件（≤512KB）、删除文档，并可绑定外部知识源（网页/RSS/向量库文件）手动或定时同步。',
      '智能问答输入"球管过热怎么处理"会自动检索知识库并给出处理步骤；"给我 ct 的故障报告"会聚合该设备的告警、异常日志、工单与维保记录。',
      '巡检产生告警时，归因信息会自动附加知识库建议，告警中心可直接看到处理指引。',
    ],
  },
  {
    title: '管家 Agent（操作执行）',
    body: [
      '巡检 Agent 兼任管家：问答中的操作类指令（如"把工单 1 转处理中"、"清理 30 天前的日志"、"立即巡检"）由管家直接执行。',
      '高危操作（删除/批量清理/移除服务/清空会话）默认两段式：管家先给出确认 token，回复"确认 <token>"后才真正执行。',
      '每次管家操作都写入 butler_audit 审计表（时间/操作/参数/结果/风险/确认状态），REST 可查。',
      '演示无人值守模式：设置环境变量 MEDOPS_BUTLER_AUTO=1 后高危操作自动执行。',
    ],
  },
  {
    title: '常见问题',
    body: [
      ...
      'Q：后端启动失败？A：查看 deploy/api.log；启动脚本会按端口自动清理占用进程。',
      'Q：启动后看不到后端窗口？A：P4c 起后端与 PostgreSQL 以隐藏窗口运行（属正常现象），停止用「medops-停止」，排障看 deploy/api.log。',
    ],
  },
]
